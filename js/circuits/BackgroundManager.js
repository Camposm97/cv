...
class Circuits {
    constructor(width, height, size, minLength, maxLength, things) {
        this.size = size;
        this.width = width;
        this.height = height;
            ...
            if (dir[0] !== 0 || dir[1] !== 0) {
                ...
                if (circuit.path.length >= minLength) {
                    ...
                    if (circuit.path.length > maxLength / 1.5) {
                        ...
                        circuit.things.push(this.things.create(circuit, speed, Math.random() * circuit.path.length * size));
                    }
...
export default class BackgroundManager {
    start() {
        function init() {
            ...
            const maxLength = 16, minLength = 3, cellSize = 10
            const circuits = new Circuits(width, height, cellSize, minLength, maxLength, things)
    
            /* draw background */
            const bg = new Background(width, height, dots, circuits)
            bgCtx.drawImage(bg.draw(), 0, 0)
        ...
        function handleWindowResize(event) {
            // console.log(`prev=${prevWidth}x${prevHeight}`)
            // console.log(`curr=${window.innerWidth}x${window.innerHeight}`)
